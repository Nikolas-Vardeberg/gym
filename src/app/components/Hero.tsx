...
export const Hero = ({ heroItemsProps }: Props) => {
  const itemsArray = Array.isArray(heroItemsProps)
    ? heroItemsProps
    : [heroItemsProps];
  return (
        ...
        <>
          <div key={x} className="relative w-full h-screen">
            ...
            <div className="absolute inset-0 container flex items-start justify-start mx-auto">
              <div className="p-4 space-y-5  absolute mx-auto bottom-[15%]">
                <h1 className="text-white font-normal text-6xl md:text-8xl">
                  {t.heading}
                </h1>
                <p className="text-white text-xl font-normal">{t.body}</p>
